...
contract DualDEXArbitrageBot is ReentrancyGuard, Ownable {
    ...
    struct ArbitrageParams {
        ...
        address tokenB;        // Target token
        uint256 amountIn;      // Input amount
        uint256 minAmountOut;  // Minimum expected output
        address dex1;          // First DEX router (buy on this DEX)
        address dex2;          // Second DEX router (sell on this DEX)
    ...
    function executeDualDEXArbitrage(
        ...
        returns (uint256 profit)
    {
        ...
        // Execute dual DEX arbitrage
        profit = _executeDualSwaps(params, cache);
    ...
    function _executeDualSwaps(
        ArbitrageParams calldata params,
        SwapCache memory cache
    ) private returns (uint256 finalProfit) {
        
        // Step 1: TokenA -> TokenB (DEX1) - Buy TokenB at lower price
        cache.intermediateBalance = _executeSingleSwap(
            params.tokenA,
            params.tokenB,
            params.amountIn,
            params.dex1,
        ...
        // Step 2: TokenB -> TokenA (DEX2) - Sell TokenB at higher price
        cache.finalBalance = _executeSingleSwap(
            params.tokenB,
            params.tokenA,
            cache.intermediateBalance,
            params.dex2,
            params.maxSlippage,
            params.deadline
        );
    ...
    function simulateDualDEXArbitrage(
        address tokenA,
        address tokenB,
        uint256 amountIn,
        address dex1,
        ...
        try this._simulateSwapChain(tokenA, tokenB, amountIn, dex1, dex2) 
        returns (uint256 finalAmount) {
            if (finalAmount > amountIn) {
                expectedProfit = finalAmount - amountIn;
                gasEstimate = 300000; // Estimated gas for dual DEX arbitrage
